...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box } body { margin:0; background:#091426; color:var(--ink); font-family:Inter, ui-sans-serif,system-ui,-apple-system,sans-serif; }
    ...
    footer { color:#8295b2;font-size:.76rem;margin-top:16px;line-height:1.45 } @media(max-width:800px){.grid{grid-template-columns:1fr}.metrics{grid-template-columns:1fr 1fr}.passengers{grid-template-columns:1fr}main{padding:17px}}
  </style>
</head>
<body><main>
  ...
  <section class="grid">
    <div class="panel"><h2>Disruption scenario</h2><div class="status"><i class="dot"></i><b>Convective weather capacity restriction at JFK</b></div>
      <div class="route"><span class="hub">SFO</span><span class="arrow">→</span><span class="hub">JFK · constrained hub</span><span class="arrow">→</span><span class="hub">LHR</span><span class="arrow">→</span><span class="hub">DEL</span></div>
      <p class="small">The optimizer evaluates an uncertain 2–6 hour weather event, reduced hub capacity, turbulence exposure, passenger connection feasibility, recovery inventory, and downstream revenue.</p>
...
function run(){ const trials=+document.querySelector('#trials').value, severity=+document.querySelector('#weather').value, risk=+document.querySelector('#risk').value; const rows=policies.map(p=>{let vals=[],pro=[];for(let i=0;i<trials;i++){let s=sample(p,severity);vals.push(s.total);pro.push(s.recovered)}const mean=vals.reduce((a,b)=>a+b,0)/trials,p95=quantile(vals,.95),tail=vals.filter(x=>x>=p95).reduce((a,b)=>a+b,0)/Math.max(1,vals.filter(x=>x>=p95).length);return {...p,mean,p95,score:mean*(1-risk)+tail*risk,protected:pro.reduce((a,b)=>a+b,0)/trials};}).sort((a,b)=>a.score-b.score);
 ...
 const volunteer=Math.max(2,Math.round(3+severity*3-Math.random()*2)), protected=Math.round(14+severity*5), standby=Math.max(1,Math.round(7-severity*3));
 document.querySelector('#overbook').innerHTML='<div><small>1. Protect</small><b>'+protected+' priority seats</b><small>Multi-leg international, medical, premium and tight-connection passengers cannot be displaced.</small></div><div><small>2. Ask volunteers</small><b>'+volunteer+' offers</b><small>Offer voucher, hotel, or same-day alternate itinerary before any involuntary step.</small></div><div><small>3. Reaccommodate</small><b>'+standby+' flexible guests</b><small>Lowest priority, local, flexible itineraries are ranked for the next safe option; compensation is included.</small></div>';
...
document.querySelector('#run').onclick=run;document.querySelector('#risk').oninput=e=>document.querySelector('#riskLabel').textContent=Math.round(e.target.value*100)+'% tail-risk weight';document.querySelector('#weather').oninput=e=>document.querySelector('#weatherLabel').textContent=Math.round(e.target.value*100)+'% forecast severity';run();
</script></body></html>`);
